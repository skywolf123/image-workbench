...
export function getPromptTextareaLayout(input: {
  ...
  const canExpand = desiredHeight > maxHeight
  const targetHeight = input.expandedHeight == null
    ? Math.min(desiredHeight, maxHeight)
    : Math.max(input.expandedHeight, minHeight)

  return {
    minHeight,
    targetHeight,
    shouldScroll: desiredHeight > targetHeight,
    canExpand,
    isSingleLine: desiredHeight <= minHeight,
  }
}
