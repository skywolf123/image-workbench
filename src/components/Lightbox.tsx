...
function LightboxInner({ src, imageId, maskPreviewSrc, onClose, showNav, currentIndex, total, onPrev, onNext, showInputActions, isInputImage, editDisabled, onReplace, onEdit, onAdvancedEdit }: LightboxInnerProps) {
  ...
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      if (e.touches.length === 2) {
        ...
        touchStartedOnControlRef.current = e.target instanceof Element && Boolean(e.target.closest('[data-lightbox-control], button'))
  ...
  return (
    <div
      ref={containerRef}
      data-lightbox-root
      className="fixed inset-0 z-[60] flex items-center justify-center overflow-hidden px-3 pb-[calc(var(--safe-area-bottom)+1rem)] pt-[calc(var(--safe-area-top)+1rem)] select-none sm:px-6 sm:py-6"
    ...
    >
      ...
      <div className="relative animate-zoom-in">
        ...
        >
          <img
            ...
            className={`saveable-image object-contain rounded-lg shadow-2xl ${showInputActions
              ? 'max-w-[90vw] max-h-[70vh] sm:max-w-[85vw] sm:max-h-[75vh]'
              : 'max-w-full max-h-[calc(100dvh-var(--safe-area-top)-var(--safe-area-bottom)-5rem)] sm:max-w-[85vw] sm:max-h-[85vh]'
            }`}
      ...
      {showInputActions && !isZoomed && (
        <div className="absolute bottom-8 left-1/2 z-10 flex w-max -translate-x-1/2 items-center gap-2 rounded-2xl bg-white/90 dark:bg-black/60 p-2 backdrop-blur-xl border border-gray-200/80 dark:border-white/15 shadow-2xl transition-colors" onClick={(e) => e.stopPropagation()}>
          ...
          <button
            type="button"
            className="flex items-center justify-center gap-2 whitespace-nowrap rounded-xl px-5 py-2.5 text-sm font-medium shadow-md transition active:scale-95 bg-emerald-500 text-white hover:bg-emerald-600 hover:shadow-emerald-500/25"
      ...
      {showNav && !isZoomed && (
        <div className={showInputActions
          ? 'absolute top-6 left-1/2 -translate-x-1/2 pointer-events-none z-10'
          : 'absolute bottom-[calc(var(--safe-area-bottom,0px)+1.25rem)] left-1/2 -translate-x-1/2 pointer-events-none'
        }>
          <span className={showInputActions
            ? 'px-3 py-1 bg-white/90 dark:bg-black/60 text-gray-800 dark:text-white/90 text-sm font-medium rounded-full backdrop-blur-md shadow-lg border border-gray-200/80 dark:border-white/15'
            : 'px-3 py-1.5 bg-black/55 text-white/90 text-xs rounded-full backdrop-blur-md ring-1 ring-white/10'
          }>
